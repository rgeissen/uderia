...
*   **15-Apr-2026:** Teradata EVS Hybrid Search — Native server-side BM25 hybrid search for server-side chunked (FILE_CONTENT_BASED) collections. The VantageCloud Lake server performs RRF fusion internally; enable via the BM25 card in the Knowledge Repository inspector. Client-side chunked collections use semantic search and show an informational note in the inspector.
