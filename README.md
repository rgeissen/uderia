...
| **Agent Packs** | Bundled agent teams (coordinator, experts, knowledge collections) as portable `.agentpack` files | Install, Fork, Publish |
...
**Discovering Assets:**
1. Navigate to the **Intelligence** pane (light bulb icon)
2. Select the product type tab (Planner Repositories, Knowledge Repositories, Agent Packs, etc.)
3. Use search and filters to find relevant assets
4. View ratings, subscriber counts, and metadata before subscribing
...
For detailed marketplace architecture, product type specifications, and API documentation, see:
[**Intelligence Marketplace Architecture (docs/Architecture/MARKETPLACE_ARCHITECTURE.md)**](docs/Architecture/MARKETPLACE_ARCHITECTURE.md)
...
4. **[Marketplace](#-intelligence-marketplace-collaborative-knowledge-sharing)** - Browse and install AI assets from the community
...
2. **[Marketplace](#-intelligence-marketplace-collaborative-knowledge-sharing)** → Browse and install AI assets for your domain
...
* **Author/Initiator:** Rainer Geissendoerfer ([LinkedIn](https://www.linkedin.com/in/rainer-geissendoerfer/))
...
*   **02-May-2026:** Continuous Knowledge Sync (CDC) — Repositories stay automatically current across environments. Only changed documents are re-processed; deleted content is removed. Supports portable Source URIs for cross-environment portability and datetime-anchor scheduling for precise sync windows.
*   **02-May-2026:** Security Architecture — Formal coverage of authentication (JWT, OAuth, SAML, OIDC), RBAC, secrets management, audit logging, threat model, and compliance mapping (EU AI Act, GDPR, SOX, HIPAA). See [Security Architecture](docs/Architecture/SECURITY_ARCHITECTURE.md).
*   **02-May-2026:** JIT Provisioning & Group Sync — User roles update automatically at every SSO login based on the user's current IdP group membership; no manual tier changes needed after org changes.
*   **02-May-2026:** SAML 2.0 Enterprise SSO — Connect to corporate identity providers via SAML 2.0; supports multiple IdP configurations, SP metadata, and both IdP- and SP-initiated login flows.
*   **01-May-2026:** Task Scheduler — Schedule any agent query to run automatically on a cron or interval; manage schedules through conversation or the split-panel UI, with results delivered by email or webhook.
*   **28-Apr-2026:** Parallel Query Execution — Multi-step queries run faster: independent plan phases and date-range iterations now execute in parallel instead of sequentially.
...
*   **23-Feb-2026:** Extensions - Post-processing transformation pipeline with `#` trigger, 6 built-in extensions (json, decision, extract, classify, summary, pdf), four-tier custom extension framework, and serial chaining for workflow automation
...
*   **15-Feb-2026:** Dual-Model Cost Breakdown - Live Status displays strategic vs tactical costs for Fusion Optimizer dual-model executions with color-coded visualization
...
*   **01-Feb-2026:** Text to Speech - Voice output now available for all profile classes with Escape-to-cancel and smart text truncation
