...
-- Version: 1.0
-- Created: 2026-01-12
-- Note: Genie profiles coordinate multiple slave profiles via LangChain
-- ============================================================================

-- ============================================================================
-- Genie Session Links
-- ============================================================================
-- Purpose: Track parent-slave session relationships for Genie coordination
-- When a Genie profile spawns slave sessions, this table records the linkage
-- to enable:
--   - UI display of slave sessions under their parent
...
CREATE TABLE IF NOT EXISTS genie_session_links (
    ...
    slave_session_id TEXT NOT NULL,        -- Slave session ID
    slave_profile_id TEXT NOT NULL,        -- Profile used for slave
    slave_profile_tag TEXT,                -- Profile tag (e.g., @CHAT, @RAG)
    user_uuid TEXT NOT NULL,               -- User who owns both sessions
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    status TEXT DEFAULT 'active',          -- active, completed, failed
    execution_order INTEGER DEFAULT 0,     -- Order in which slave was invoked

    -- Ensure unique parent-slave relationship
    UNIQUE(parent_session_id, slave_session_id)
);

-- Index for finding all slaves of a parent session
CREATE INDEX IF NOT EXISTS idx_genie_parent
ON genie_session_links(parent_session_id);

-- Index for finding parent of a slave session
