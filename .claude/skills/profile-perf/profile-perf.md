...
Lower-intelligence models can outperform higher-intelligence models for structured MCP prompt workflows due to the Fusion Optimizer's orchestrator-driven architecture. Use profile-perf to compare models on the same query and measure token efficiency, self-correction count, JSON error rate, and cost.

**Full analysis and guidelines:** See fusion-hardening skill **Section 5.1: Model Selection for Structured Workflows**
