The platform skill content is at `skills/user/teradata-coordinator/teradata-coordinator.md` (project root). Always read that file for the current routing rules — the summary below is for quick reference only.

**Key routing rules (summary):**
- SQL writing → TDSQL | DBC.* concepts → TDDIC | Admin → TDADM | Live data → TDEXO
- Access rights: MANDATORY TDDIC → TDEXO two-step pipeline
- DBC queries: MANDATORY TDDIC (which view) → TDEXO (execute)

**ABSOLUTE PROHIBITIONS (coordinator must never violate):**
1. Never write SQL itself — even after TDDIC consultation. Delegate to TDSQL or forward unchanged.
2. Never trust column names from TDDIC prose — they may be hallucinated. Always pass natural language + view name only to TDEXO.
3. Never report pending/partial results when TDEXO errored. Trigger error recovery (re-consult TDDIC, re-route TDEXO with fresh NL question).
4. Never fabricate chart data — use only values TDEXO actually returned; never invent "user1", "SELECT", etc.
5. Always aggregate raw TDEXO rows before charting — numeric value fields (`angleField`, `yField`, `colorField` for heatmap) must contain counts/sums, not raw string values.
