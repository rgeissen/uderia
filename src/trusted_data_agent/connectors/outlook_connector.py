...
via Admin Panel → Components → Connectors → uderia-outlook (credentials section).

Users connect their own Microsoft account via:
  Platform Components → Connectors → uderia-outlook → "Connect Microsoft account"

Graph API scopes:
  Mail.Read  Mail.Send  Mail.ReadWrite  Calendars.ReadWrite  Contacts.Read
...
def initiate_oauth(user_uuid: str, redirect_uri: str) -> Optional[str]:
    ...
    if not client_id:
        logger.warning("Outlook connector: AZURE_CLIENT_ID not configured.")
        return None
...
async def handle_callback(code: str, state: str, redirect_uri: str) -> Optional[dict]:
    ...
    if not state_data:
        logger.warning("Outlook connector: Invalid or expired OAuth state token.")
        return None
    ...
    if token_resp.status_code != 200:
        logger.error(f"Outlook token exchange failed ({token_resp.status_code}): {token_resp.text}")
        return None
    ...
    if not access_token:
        logger.error("Outlook token exchange returned no access_token.")
        return None
    ...
    try:
        ...
        logger.warning(f"Could not fetch Outlook user info: {exc}")
    ...
    logger.info(f"Outlook account connected for user {user_uuid}: {display_name}")
...
async def _do_refresh(user_uuid: str, refresh_token: str) -> bool:
    ...
    try:
        ...
        logger.warning(f"Outlook token refresh HTTP error: {exc}")
        return False

    if resp.status_code != 200:
        logger.warning(f"Outlook token refresh failed ({resp.status_code}): {resp.text}")
        return False
...
async def disconnect(user_uuid: str):
    with _get_conn() as conn:
        ...
        conn.commit()
    logger.info(f"Outlook account disconnected for user {user_uuid}")
...
def inject_env_tokens(user_uuid: str, env: dict):
    try:
        ...
        if not row:
            return
        if row["access_token"]:
            env["MS_OUTLOOK_ACCESS_TOKEN"] = _decrypt(row["access_token"])
        if row["refresh_token"]:
            env["MS_OUTLOOK_REFRESH_TOKEN"] = _decrypt(row["refresh_token"])
    except Exception as exc:
        logger.warning(f"Could not inject Outlook tokens for user {user_uuid}: {exc}")
