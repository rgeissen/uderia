"""
Microsoft Teams OAuth connector.
...
via Admin Panel → Components → Connectors → uderia-teams (credentials section).

Users connect their own Microsoft account via:
  Platform Components → Connectors → uderia-teams → "Connect Microsoft account"

Graph API scopes:
  Team.ReadBasic.All  Channel.ReadBasic.All  ChannelMessage.Read.All
  ChannelMessage.Send  OnlineMeetings.ReadWrite  User.Read  offline_access
"""
...
def _get_admin_creds() -> Tuple[Optional[str], Optional[str], str]:
    ...
    # Tenant from config; default "common" allows personal + work accounts
    tenant = "common"
...
def initiate_oauth(user_uuid: str, redirect_uri: str) -> Optional[str]:
    ...
    if not client_id:
        logger.warning("Teams connector: AZURE_CLIENT_ID not configured.")
        return None
...
async def handle_callback(code: str, state: str, redirect_uri: str) -> Optional[dict]:
    ...
    if not state_data:
        logger.warning("Teams connector: Invalid or expired OAuth state token.")
        return None
    ...
    if token_resp.status_code != 200:
        logger.error(f"Teams token exchange failed ({token_resp.status_code}): {token_resp.text}")
        return None
    ...
    if not access_token:
        logger.error("Teams token exchange returned no access_token.")
        return None
    ...
    try:
        ...
        if me_resp.status_code == 200:
            data = me_resp.json()
            display_name = data.get("userPrincipalName") or data.get("displayName") or data.get("mail")
    except Exception as exc:
        logger.warning(f"Could not fetch Teams user info: {exc}")
    ...
    logger.info(f"Teams account connected for user {user_uuid}: {display_name}")
...
async def _do_refresh(user_uuid: str, refresh_token: str) -> bool:
    ...
    try:
        ...
        logger.warning(f"Teams token refresh HTTP error: {exc}")
        return False

    if resp.status_code != 200:
        logger.warning(f"Teams token refresh failed ({resp.status_code}): {resp.text}")
        return False
...
async def disconnect(user_uuid: str):
    with _get_conn() as conn:
        ...
        conn.commit()
    logger.info(f"Teams account disconnected for user {user_uuid}")
...
def inject_env_tokens(user_uuid: str, env: dict):
    try:
        ...
        if not row:
            return
        if row["access_token"]:
            env["MS_TEAMS_ACCESS_TOKEN"] = _decrypt(row["access_token"])
        if row["refresh_token"]:
            env["MS_TEAMS_REFRESH_TOKEN"] = _decrypt(row["refresh_token"])
    except Exception as exc:
        logger.warning(f"Could not inject Teams tokens for user {user_uuid}: {exc}")
